...
class DataGenerator_Mel_loudness_graph(object):
    ...
    def generate_inference_soundscape_clip_for_LLM(self, Dataset_mel, Dataset_loudness):
        ...
        for file in os.listdir(Dataset_mel):
            ...
            print('Inference audio clip mel SHAPE: ', file, data.shape)
        ...
        for file in file_names:
            ...
            data = np.load(file_path, allow_pickle=True)    
            print('Inference audio clip loudness SHAPE: ', file, data.shape)
        ...
        self.test_x_loudness = np.zeros((self.test_x.shape[0], 15000, 1), dtype=np.float32)       # DUMMY 
    ...
    def transform(self, x, mean, std):
        ...
        # If dtype is string, try converting to float
        if np.issubdtype(x.dtype, np.str_) or np.issubdtype(x.dtype, np.object_):
            try:
                x = x.astype(np.float32)
            except ValueError:
                raise ValueError("Input contains non-convertible strings to float.")
