...
class DataGenerator_Mel_loudness_no_graph(object):
    def __init__(self, Dataset_path, batch_size=config.batch_size, seed=42, normalization=True, overwrite=False,
        ...
        if self.normal and not os.path.exists(normalization_loudness_file) or overwrite:
            ...
            if using_mel:
                ...
                print('Mean: ', self.mean_log_mel)
                print('STD: ', self.std_log_mel)
    ...
    def get_input_output(self, all_data):
        ...
        scene_labels = np.zeros(len(sound_maskers_labels))
        ISOPls = np.zeros((len(sound_maskers_labels), 1))
        ISOEvs = np.zeros((len(sound_maskers_labels), 1))
    ...
    def generate_train(self):
        ...
        while True:
            ...
            if self.using_mel:
                yield batch_x, batch_scene, batch_event, batch_ISOPls, batch_ISOEvs, \
                      batch_pleasant, batch_eventful, batch_chaotic, batch_vibrant, \
                      batch_uneventful, batch_calm, batch_annoying, batch_monotonous

            if self.using_loudness:
                yield batch_x_loudness, batch_scene, batch_event, batch_ISOPls, batch_ISOEvs, \
                      batch_pleasant, batch_eventful, batch_chaotic, batch_vibrant, \
                      batch_uneventful, batch_calm, batch_annoying, batch_monotonous

            if self.using_loudness and self.using_mel:
                yield batch_x, batch_x_loudness, batch_scene, batch_event, batch_ISOPls, batch_ISOEvs, \
                      batch_pleasant, batch_eventful, batch_chaotic, batch_vibrant, \
                      batch_uneventful, batch_calm, batch_annoying, batch_monotonous
    ...
    def generate_testing(self, data_type, max_iteration=None):
        ...
        if self.using_mel:
            ...
            all_feature_file_path = os.path.join(self.Dataset_path, 'CASE_STUDY', '_hop_160_npy.pickle') # DEZE VERANDEREN NOG
